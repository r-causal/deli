...
test_that("ee_ipw exact mode matches the Python reference", {
  ...
  m <- estimate(MEstimator(stacked_equations = psi, init = ref$init),
                deriv_method = "exact")
...
test_that("ee_aipw exact mode matches the Python reference", {
  ...
  psi <- function(theta) {
    ee_aipw(theta, y = ref$y, A = ref$a, W = ref$W,
            X = ref$X, X1 = ref$X1, X0 = ref$X0)
  }
  m <- estimate(MEstimator(stacked_equations = psi, init = ref$init),
                deriv_method = "exact")
...
test_that("ee_gestimation_snmm exact mode matches the Python reference", {
  ...
  psi <- function(theta) {
    ee_gestimation_snmm(theta, y = ref$y, A = ref$a, W = ref$W, V = ref$V,
                        model = ref$model)
  }
  m <- estimate(MEstimator(stacked_equations = psi, init = ref$init),
                deriv_method = "exact")
...
test_that("ee_rogan_gladen_extended exact mode matches the Python reference", {
  ...
  psi <- function(theta) {
    ee_rogan_gladen_extended(theta, y = ref$y, y_star = ref$y_star,
                             r = ref$r, X = ref$X)
  }
  m <- estimate(MEstimator(stacked_equations = psi, init = ref$init),
                deriv_method = "exact")
...
test_that("ee_regression_calibration exact mode matches the Python reference", {
  ...
  psi <- function(theta) {
    ee_regression_calibration(theta, beta = ref$beta, a = ref$a,
                              a_star = ref$a_star, r = ref$r)
  }
  m <- estimate(MEstimator(stacked_equations = psi, init = ref$init),
                deriv_method = "exact")
...
test_that("exact and capprox derivatives agree for the causal equations", {
  ...
  expect_exact_matches_capprox_cm(
    function(t) ee_ipw(t, y = ipw$y, A = ipw$a, W = ipw$W), ipw$init
  )
  ...
  expect_exact_matches_capprox_cm(
    function(t) ee_gestimation_snmm(t, y = gv$y, A = gv$a, W = gv$W, V = gv$V,
                                    model = gv$model),
    gv$init
  )
  ge_eff <- load_fixture("ee_gestimation_efficient_exact")
  expect_exact_matches_capprox_cm(
    function(t) ee_gestimation_snmm(t, y = ge_eff$y, A = ge_eff$a, W = ge_eff$W,
                                    V = ge_eff$V, X = ge_eff$X,
                                    model = ge_eff$model),
    ge_eff$init
  )
