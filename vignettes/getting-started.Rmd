...
from the row names of an estimating function that returns them, and the
assignment is ignored while a value carries derivatives, because the labels are
read from the plain evaluation at the solved values instead. `rownames<-` hands
its work to `dimnames<-`, which is a generic, so deli registers the setter there
and an estimating function can label its rows from anywhere.
