...
without any additional arguments. See `?ee_regression` for the full set of
supported models, including robust, ridge, LASSO, elastic net, beta, and tobit
regression.
...
m <- MEstimator(stacked_equations = psi, init = mean(y)) |> estimate()
...
no step size to tune, and it is the method Python Delicatessen uses internally.
...
operators and the elementwise math functions (the `Ops` and `Math` group
members, including `exp`, `log`, `sqrt`, and the trigonometric functions), the
matrix product `%*%`, the transpose `t()`, indexing, the logistic transforms
`inverse_logit()` and `logit()`, and the `lgamma` / `digamma` / `trigamma` /
`polygamma` family.
...
- `?ee_regression`: GLM, robust, ridge, LASSO, elastic net, beta, and tobit
  regression
