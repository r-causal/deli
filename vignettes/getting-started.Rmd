...
code that requires a plain number, so the call fails with a `non-numeric
argument to mathematical function` error instead of returning a derivative.
deli exports a counterpart for each one:
...
Arithmetic, `%*%`, `t()`, `[`, and `c()` are the operations that always carry
derivatives, because deli registers each as an S3 method and S3 dispatch does
not depend on where the call is written. Build a p-by-n return with
`t(X * resid)`, the idiom `ee_regression()` itself uses. Use `c()` to flatten a
matrix-shaped result such as `X %*% theta` down to a vector; `as.numeric()`
cannot do that job under exact differentiation, because it has to return a
plain double and so cannot carry a derivative, and it aborts rather than
silently returning a zero one.
...
`mean()` is the quietest of these traps. It dispatches to `mean.default()`,
which sits outside every group generic deli supports, so a tangent-carrying
argument produces the warning `argument is not numeric or logical: returning
NA` and a derivative of zero rather than an error. Write `sum(x) / length(x)`
instead, which differentiates exactly because both `sum()` and `length()` do.
