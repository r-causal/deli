...
those helpers, and rely on the finite-difference default otherwise. Combining
values with `c()` fails for a related reason: it returns a plain list rather
than a differentiable vector, so a later `sum()` over the result errors. Index
into `theta` directly instead.
