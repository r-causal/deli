...
m <- MEstimator(stacked_equations = psi, init = c(0, 0, 0, 0, 0, 0, 0)) |>
  estimate()
...
m <- MEstimator(stacked_equations = psi, init = c(0, 0, 0, 0, 0, 0)) |>
  estimate()
...
m_gformula <- MEstimator(stacked_equations = psi_gformula, init = rep(0, 7)) |>
  estimate()

psi_ipw <- function(theta) {
  ee_ipw(theta, y = Y, A = A, W = cbind(1, W1, W2))
}
m_ipw <- MEstimator(stacked_equations = psi_ipw, init = rep(0, 6)) |>
  estimate()
...
m_aipw <- MEstimator(stacked_equations = psi_aipw, init = rep(0, 10)) |>
  estimate()
