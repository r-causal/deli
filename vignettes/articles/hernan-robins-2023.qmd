...
ci <- confidence_intervals(estr)
cat("qsmk:", round(estr@theta[2], 2), "\n")
cat("95% CI:", round(ci[2, 1], 2), "to", round(ci[2, 2], 2), "\n")
```
...
estr <- MEstimator(
  ...
  init = c(0, 0, rep(0, ncol(W)))
)
estr <- estimate(estr, solver = "nleqslv", maxiter = 5000)

ci <- confidence_intervals(estr)
cat("alpha_0:", round(estr@theta[1], 2), "\n")
cat("alpha_1 (ACE):", round(estr@theta[2], 2),
    "95% CI:", round(ci[2, 1], 2), "to", round(ci[2, 2], 2), "\n")
```
...
estr <- estimate(estr, solver = "nleqslv", tolerance = 1e-12, maxiter = 5000)

ci <- confidence_intervals(estr)
cat("MSM coefficients:\n")
for (i in 1:3) {
  cat("  theta_", i, ":", round(estr@theta[i], 3),
      "95% CI:", round(ci[i, 1], 3), "to", round(ci[i, 2], 3), "\n")
}
```
...
cat("No change in smoking\n")
cat(pred_y$predicted[1], "\n")
cat("95% CI:", pred_y$lower[1], "to", pred_y$upper[1], "\n\n")
cat("+20 smoking intensity\n")
cat(pred_y$predicted[2], "\n")
cat("95% CI:", pred_y$lower[2], "to", pred_y$upper[2], "\n")
```
...
ci <- confidence_intervals(estr)
cat("Causal OR for qsmk:", round(exp(estr@theta[2]), 4), "\n")
cat("95% CI:", round(exp(ci[2, 1]), 4), "to", round(exp(ci[2, 2]), 4), "\n")
```
...
estr <- MEstimator(
  ...
  init = c(0, 0, 0, 0, 0, rep(0, ncol(W)))
)
estr <- estimate(estr, solver = "nleqslv", maxiter = 5000)

ci <- confidence_intervals(estr)
cat("MSM with effect modification by sex:\n")
for (i in 1:4) {
  cat("  theta_", i, ":", round(estr@theta[i], 2),
      "95% CI:", round(ci[i, 1], 2), "to", round(ci[i, 2], 2), "\n")
}
```
...
estr <- estimate(estr, maxiter = 5000)
...
estr <- MEstimator(
  ...
  init = rep(0, ncol(X_out))
)
estr <- estimate(estr, solver = "nleqslv", maxiter = 5000)

# Save starting values for later use
init_reg <- estr@theta
```
...
estr <- MEstimator(
  ...
  init = c(0, 0, 0, init_reg)
)
estr <- estimate(estr, solver = "nleqslv", maxiter = 5000)

ci <- confidence_intervals(estr)
cat("ACE:", round(estr@theta[1], 2),
    "95% CI:", round(ci[1, 1], 2), "to", round(ci[1, 2], 2), "\n")
cat("E[Y^1]:", round(estr@theta[2], 2),
    "95% CI:", round(ci[2, 1], 2), "to", round(ci[2, 2], 2), "\n")
cat("E[Y^0]:", round(estr@theta[3], 2),
    "95% CI:", round(ci[3, 1], 2), "to", round(ci[3, 2], 2), "\n")
```
...
estr <- MEstimator(
  ...
  init = c(0, 0, 0, rep(0, ncol(X_out)), rep(0, ncol(W)))
)
estr <- estimate(estr, solver = "nleqslv", maxiter = 5000)
...
estr <- MEstimator(
  ...
  init = c(0, 0, 0, rep(0, ncol(W)), -1.5, rep(0, ncol(X_out) - 1))
)
estr <- estimate(estr, solver = "nleqslv", maxiter = 5000)
...
ci <- confidence_intervals(estr)
cat("SNM phi_0:", round(estr@theta[1], 2),
    "95% CI:", round(ci[1, 1], 2), "to", round(ci[1, 2], 2), "\n")

# Save theta for later use
snm1_init <- as.numeric(estr@theta)
```
...
ci <- confidence_intervals(estr)
cat("SNM phi_0 (built-in):", round(estr@theta[1], 2),
    "95% CI:", round(ci[1, 1], 2), "to", round(ci[1, 2], 2), "\n")
```
...
cat("SNM phi_0:", round(estr@theta[1], 2),
    "95% CI:", round(ci[1, 1], 2), "to", round(ci[1, 2], 2), "\n")
cat("SNM phi_1:", round(estr@theta[2], 2),
    "95% CI:", round(ci[2, 1], 2), "to", round(ci[2, 2], 2), "\n")
```
...
ci <- confidence_intervals(estr)
cat("Pr(A=1|Z=1):", round(estr@theta[1], 3), "\n")
cat("Pr(A=1|Z=0):", round(estr@theta[2], 3), "\n")
cat("Risk difference:", round(estr@theta[3], 3),
    "95% CI:", round(ci[3, 1], 3), "to", round(ci[3, 2], 3), "\n")
```
...
ci <- confidence_intervals(estr)
cat("IV estimate:", round(estr@theta[1], 3),
    "95% CI:", round(ci[1, 1], 3), "to", round(ci[1, 2], 3), "\n")
```
...
ci <- confidence_intervals(estr)
cat("IV estimate (built-in):", round(estr@theta[1], 3),
    "95% CI:", round(ci[1, 1], 3), "to", round(ci[1, 2], 3), "\n")
```
...
ci <- confidence_intervals(estr)
cat("2SLS IV estimate:", round(estr@theta[1], 2),
    "95% CI:", round(ci[1, 1], 2), "to", round(ci[1, 2], 2), "\n")
```
...
ci <- confidence_intervals(estr)
cat("2SLS (built-in) IV estimate:", round(estr@theta[1], 2),
    "95% CI:", round(ci[1, 1], 2), "to", round(ci[1, 2], 2), "\n")
```
...
```{r}
#| label: ch16-5
for (c_val in c(1.60, 1.70, 1.80, 1.90)) {
  z_new <- as.integer(df_iv$price82 >= c_val)
  estr <- MEstimator(
    stacked_equations = function(theta) {
      ee_2sls(theta, y = df_iv$wt82_71, A = df_iv$qsmk,
