...
init_ps <- unname(coef(glm(a ~ W - 1, family = binomial())))
```
...
estr <- MEstimator(
  ...
  estimate(solver = "nleqslv", maxiter = 5000)
...
estr <- MEstimator(
  ...
  estimate(solver = "nleqslv", tolerance = 1e-12, maxiter = 5000)

# MSM coefficients
round(cbind(estimate = coef(estr), confint(estr))[1:3, ], 3)
```
...
round(cbind(estimate = coef(estr), confint(estr))[1:4, ], 2)
```
