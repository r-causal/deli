...
estr <- MEstimator(stacked_equations = psi, init = c(-40, 5))
estr <- estimate(estr)

# View results
estr@theta
sqrt(diag(estr@variance))
confidence_intervals(estr)
```
...
estr <- estimate(estr, solver = "nleqslv")
...
# Estimate the parameters
estr_null <- MEstimator(stacked_equations = psi, init = c(0))
estr_null <- estimate(estr_null)
estr_null@theta
```
