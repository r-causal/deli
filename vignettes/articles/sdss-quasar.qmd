...
# Estimate the model
init_vals <- rep(0, 12)
estr <- MEstimator(stacked_equations = psi, init = init_vals) |>
  estimate()
```
...
plot(d$z, d$u_mag - d$g_mag, pch = ".", col = adjustcolor("lightblue", alpha.f = 0.2),
     xlab = "Redshift (z)", ylab = "u - g")
lines(p$z, yhat$predicted, col = "black")
```
...
cov_p <- Xa_pred %*% estr@variance %*% t(Xa_pred)
...
plot(d$z, d$u_mag - d$g_mag, pch = ".", col = adjustcolor("lightblue", alpha.f = 0.2),
     xlab = "Redshift (z)", ylab = "u - g")
polygon(c(p$z, rev(p$z)), c(cb[, "lower"], rev(cb[, "upper"])),
...
# Plotting
plot(d$z, bshift, pch = ".", col = adjustcolor("deeppink", alpha.f = 0.2),
     xlab = "Redshift (z)", ylab = "g - r")
polygon(c(p$z, rev(p$z)), c(cb[, "lower"], rev(cb[, "upper"])),
        col = adjustcolor("black", alpha.f = 0.4), border = NA)
lines(p$z, yhat$predicted, col = "black")
```
...
# Plotting
plot(d$z, bshift, pch = ".", col = adjustcolor("springgreen", alpha.f = 0.2),
     xlab = "Redshift (z)", ylab = "g - i")
polygon(c(p$z, rev(p$z)), c(cb[, "lower"], rev(cb[, "upper"])),
        col = adjustcolor("black", alpha.f = 0.4), border = NA)
lines(p$z, yhat$predicted, col = "black")
```
