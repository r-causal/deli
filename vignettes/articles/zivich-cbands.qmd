---
title: "Zivich et al. (2025): Confidence Bands"
knitr:
  opts_chunk:
    collapse: true
    comment: '#>'
...
[Zivich et al. (2025): An Introduction to Confidence Bands example](https://deli.readthedocs.io/en/latest/Examples/Zivich-CBands.html)
...
estr <- MEstimator(stacked_equations = estfunc, init = inits) |> estimate()
psi <- estr@theta
...
plot(NULL, xlim = c(-10, 80), ylim = c(-70, 70),
     xlab = expression("Average Causal Effect - CD4, " * psi[1]),
     ylab = expression("Average Causal Effect - CD8, " * psi[2]))

# Reference lines for the null
abline(h = 0, lty = 3, col = "gray")
abline(v = 0, lty = 3, col = "gray")

# Confidence interval rectangle (implied)
rect(ci[1, 1], ci[2, 1], ci[1, 2], ci[2, 2],
     border = adjustcolor("red", alpha.f = 0.3), lty = 2)

# Confidence intervals as lines
lines(ci[1, ], rep(psi[2], 2), col = "red")
lines(rep(psi[1], 2), ci[2, ], col = "red")

# Confidence bands rectangle
rect(cb[1, 1], cb[2, 1], cb[1, 2], cb[2, 2], border = "blue")

# Estimated parameters
points(psi[1], psi[2], pch = 18, cex = 1.5, col = "black")

legend("topleft", legend = c("Intervals", "sup-t Band", expression(hat(theta))),
       col = c("red", "blue", "black"), lty = c(1, 1, NA),
       pch = c(NA, NA, 18), bg = "white")
```
...
For the second example, we are interested in studying modification of the effect of antiretroviral therapy on CD4 by gender. Here, we will estimate the following marginal structural model
...
plot(cd4_grid, y_vals, type = "l", col = "black",
     xlim = c(90, 930), ylim = c(150, 700),
     xlab = "Baseline CD4",
     ylab = "Conditional Average Causal Effect")
grid()

# Confidence bands shading
polygon(c(cd4_grid, rev(cd4_grid)),
        c(cb[, 1], rev(cb[, 2])),
        col = adjustcolor("blue", alpha.f = 0.4), border = NA)

# Confidence intervals shading
polygon(c(cd4_grid, rev(cd4_grid)),
        c(y_hat$lower, rev(y_hat$upper)),
        col = adjustcolor("red", alpha.f = 0.4), border = NA)

# Prediction line on top
lines(cd4_grid, y_vals, col = "black")

legend("topleft", legend = c("Bands", "Intervals"),
       fill = adjustcolor(c("blue", "red"), alpha.f = 0.4), border = NA)
```
