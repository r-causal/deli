...
# Fit the M-estimator
estr <- MEstimator(stacked_equations = psi, init = c(0.01))
estr <- estimate(estr)
...
results <- data.frame(
  ...
  LCL = confidence_intervals(estr)[, 1],
  UCL = confidence_intervals(estr)[, 2],
  row.names = c("Lambda")
)
round(results, 3)
```
...
# Fit the M-estimator
estr <- MEstimator(stacked_equations = psi, init = c(0.01, 1.0))
estr <- estimate(estr)
...
# Fit the M-estimator
estr <- MEstimator(stacked_equations = psi, init = c(6.0, 0.0))
estr <- estimate(estr)
